...
description: "Choose single-agent or team execution for runTeam with explicit modes, governance policy, custom routers, and auditable routing decisions."
---

Execution Routing decides the execution topology for an automatic `runTeam()` call: use one Agent directly (`single`) or ask the Coordinator to build and execute a Team plan (`team`). It is orthogonal to [Model Routing](/reference/model-routing/), which chooses the model for calls inside the selected topology.
...
to clear it are in
[errors](https://github.com/open-multi-agent/open-multi-agent/blob/v1.20.0/docs/errors.md#routingdeclarationrequirederror).
...
[five routing trace source classifications](/reference/observability/#trace-spans),
