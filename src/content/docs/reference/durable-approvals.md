...
description: "Suspend a checkpointed run at a plan, round, dispatch, or tool-call gate, decide out of band, then resume from exactly the reviewed content."
---
...
approved run. Add a [run store](/reference/run-store/) when more than one worker can pick
...
[`durable-approval`](https://github.com/open-multi-agent/open-multi-agent/blob/v1.20.0/packages/core/examples/patterns/durable-approval.ts)
