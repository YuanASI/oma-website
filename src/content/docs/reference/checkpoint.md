...
description: "Opt-in per-run snapshots over any MemoryStore — persist task progress and resume after a crash, abort, or restart with restore()."
---

Long-running task workflows can persist their progress and resume after a crash, an abort, or a process restart. Checkpointing is **opt-in** and runs entirely over the existing [`MemoryStore`](/reference/shared-memory/) interface, so the same in-memory, Redis, Postgres, or custom backend that holds shared memory also holds checkpoints — no extra storage layer.
...
[durable approval gates](/reference/durable-approvals/). Approval decisions live in
...
- **Journal watermark (schema v5 only)** — `journalWatermarkSeq`, the highest
  [run journal](/reference/run-journal/) sequence this snapshot folds, plus an
...
closed. See [durable approvals](/reference/durable-approvals/#rejection-and-recovery-semantics).

An enabled [run store](/reference/run-store/) adds a second exception: the checkpoint
...
[Run journal](/reference/run-journal/) appends follow the same contract, with no
...
- **Snapshot-based, not event-sourced.** Each checkpoint overwrites the previous one. Enabling a [run journal](/reference/run-journal/) adds a replayable tail after the latest snapshot ([Tail replay](#tail-replay)), but the snapshot remains the anchor and the journal is never required to recover.
- **No execution ownership on its own.** A checkpoint is state, not a claim on
  ...
  [run store](/reference/run-store/) for a run-level lease and fencing token. Without one,
...
Append-only transition replay shipped as the opt-in [run journal](/reference/run-journal/) ([#527](https://github.com/open-multi-agent/open-multi-agent/issues/527)), which adds the tail replay described above and lets `verifyRun()` audit a finished run offline. The earlier proposal to replace snapshots with it outright ([#313](https://github.com/open-multi-agent/open-multi-agent/issues/313)) stays closed: the snapshot remains the recovery anchor.
