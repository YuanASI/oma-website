---
title: "Run store and execution leases"
description: "An opt-in authoritative record per logical run: lifecycle status, an execution lease, and a fencing token, so one worker at a time advances it."
---

A [checkpoint](/reference/checkpoint/) answers *what state can execution resume from*. It
...
[external-side-effect idempotency window](/reference/checkpoint/#mid-task-tool-recovery),
...
An ordinary [checkpoint write is best-effort](/reference/checkpoint/#saves-are-best-effort):
...
`MemoryStoreRunStore` adapts any [`MemoryStore`](/reference/shared-memory/) that
...
- **Not an event log.** The record is the current authoritative state, not a
  history. Append-only history is the opt-in [run journal](/reference/run-journal/).
...
- **Not an approval product.** Reviewer UI, RBAC, notification, and escalation
  ...
  [durable approvals](/reference/durable-approvals/#what-the-framework-provides-and-what-you-build).

## Related

- [Checkpoint and resume](/reference/checkpoint/) — the snapshot a lease protects.
- [Durable approval gates](/reference/durable-approvals/) — what a suspended run is waiting on.
- [Observability](/reference/observability/#dashboard-tracestore-checkpointstore-and-runstore) — how this data plane differs from telemetry.
- [Shared memory](/reference/shared-memory/) — the `MemoryStore` interface the bundled adapter sits on.
