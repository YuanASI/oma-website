---
title: "Model routing"
description: "Opt-in deterministic policy that routes orchestration phases to different models — match by phase, agent, task role, priority, or leaf."
---

Model Routing chooses models for calls inside an execution topology. It is orthogonal to [Execution Routing](/reference/execution-routing/), which decides whether an automatic `runTeam()` call uses one Agent or a Coordinator-generated Team plan.
...
[`examples/patterns/cost-tiered-pipeline.ts`](https://github.com/open-multi-agent/open-multi-agent/blob/v1.13.0/packages/core/examples/patterns/cost-tiered-pipeline.ts) runs the same four-stage pipeline twice (all-flagship vs. a tiered mix) and prints the per-model token and USD breakdown, so you can see what a routing policy saves before adopting one.
