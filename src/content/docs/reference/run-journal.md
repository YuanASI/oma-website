---
title: "Run event journal"
description: "An opt-in append-only log of what entered the context, which tools ran, and how the plan moved — the audit trail behind why the model saw what it saw."
---
...
The journal is not the recovery mechanism. [Checkpoint snapshots](/reference/checkpoint/) remain the durable recovery anchor; the journal adds an audit trail alongside them and, on restore, a replayable tail after the last snapshot.
...
Redaction uses the same option shape as [`RedactingStore`](/reference/shared-memory/) and runs at write time, so `readFrom` returns what was persisted:
...
**Restored runs keep their lineage too.** A run resumed from a v5 checkpoint does not re-emit the conversation the previous attempt journaled — that would duplicate the events the seqs point at. Instead the snapshot carries per-block lineage positionally and restore re-attaches it to the parsed blocks, and any events the journal recorded after the snapshot's watermark are folded in with lineage of their own. See [tail replay](/reference/checkpoint/#tail-replay). Resuming from a v4 or older snapshot has no persisted lineage to re-attach, so those blocks are recorded as the gaps they are — and `enforceLineage: true` fails such a resume, correctly.
...
This holds at approval boundaries too, where ordinary checkpoint saves escalate. Durability there is the [durable approval ledger](/reference/durable-approvals/)'s job; the journal only records that the boundary existed. Losing the audit trail must never roll back a run that actually happened.
...
Human identity enters the record in exactly two places, both downstream of a [durable approval](/reference/durable-approvals/):
