...
description: "What the framework connects to, where state lives, and what an air-gapped deployment needs — the facts a security review asks for, in one place."
---
...
means the local [observability](/reference/observability/) subsystem: trace records that
...
- **`process` and `acp` backends** spawn the command you name. See
  [external agents](/reference/external-agents/).
...
[run records](/reference/run-store/) all go through the `MemoryStore` interface, and core
...
[durable approvals](/reference/durable-approvals/#store-requirements), where the
decision write is the correctness boundary, and for the
[run store](/reference/run-store/), where it decides whether two workers can advance the
...
[shared memory](/reference/shared-memory/#redacting-persisted-secrets).
...
1. **An OpenAI-compatible endpoint.** Configure it as `provider: 'openai'` with
   ...
   ignores it. See
   [local model tool-calling](/reference/providers/#local-model-tool-calling).
...
- **Tamper-evident audit.** The run journal has no hash chain, no signature,
  ...
  [identity and integrity](/reference/run-journal/#identity-and-integrity).
