...
4. ### 切换到真实模型
   ...
   Cloud 或 OpenAI-compatible Starter 先复制环境变量模板：

   ```bash
   cp .env.example .env
   ```

   在 `.env` 中填写凭证和模型配置，然后启动运行：
...
<CardGrid>
	<LinkCard
		title="选择运行方式"
		href="/zh/getting-started/three-ways-to-run/"
		description="在 runAgent()、runTeam() 和 runTasks() 之间做选择。"
	/>
	<LinkCard
		title="示例"
		href="/zh/examples/"
		description="运行或改造完整工作流。"
	/>
	<LinkCard
		title="模型提供方"
		href="/zh/reference/providers/"
		description="配置托管、OpenAI-compatible 或本地模型。"
	/>
</CardGrid>
