...
无效的已提交载荷、不受支持的文件格式或不受支持的 trace schema 都会大声地在打开时
失败；它绝不会被跳过。
...
- `flush()` 会排在它之前被接受的所有操作之后等待，并对目标文件执行 `fsync`。重复
  调用是安全的。在本地文件系统所承诺的范围内，它是挺过操作系统崩溃/断电的显式边界。
...
`diagnostics: 'silent'` 可禁用警告，或使用 `onDiagnostic`。诊断处理器抛出的异常会被
吞掉，而不会递归地产生另一条诊断。
...
数据库——但要先判断哪些数据进那个 sink 是安全的。可运行的示例见
