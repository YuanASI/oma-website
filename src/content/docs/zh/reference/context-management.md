...
- 某些本地 OpenAI 兼容模型可能把 `<thinking>` 文本重新输出到自己的 assistant 回复中，这可能触发循环检测器。失败模式与缓解办法见 [`examples/patterns/cross-provider-reasoning.ts`](https://github.com/open-multi-agent/open-multi-agent/blob/v1.13.0/packages/core/examples/patterns/cross-provider-reasoning.ts)。
