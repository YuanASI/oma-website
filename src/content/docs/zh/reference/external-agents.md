...
用 `backend` 而非模型来声明一个智能体。团队的其他一切都保持不变：
...
OMA 担任 ACP **客户端**角色。在某个智能体的第一次运行时，它启动子进程，把它的 stdio 组织为以换行分隔的 JSON-RPC，执行 `initialize`，并在 `cwd` 中打开一个 `session/new`。随后每次 `agent.run(prompt)` 发送一个 `session/prompt` 回合，并将 `session/update` 通知汇入一个正常的智能体结果：
...
本次发布**尚未**做到的（附上真实用例开一个 issue，可推动其中任何一项提前实现）：
