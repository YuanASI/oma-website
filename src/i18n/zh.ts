...
export const zh: UiDict = {
  ...
  landing: {
    ...
    sectionWhy: {
      eyebrow: '为什么选 OMA',
      title: '从意图到一次可控、可检查的运行。',
      sub: '选择执行拓扑，让就绪工作立即移动，在正确的边界设闸或挂起，恢复并修订，再留下可以核验的证据。',
    },
    why: [
      {
        tag: '规划 → 路由',
    ...
    whyViewer: {
      ...
      body: '离线 Run Viewer 把一次已完成的运行变成可复查的证据，无需上传到托管的 OMA 服务。开启运行事件日志后，verifyRun() 还能仅凭日志里的事件再核一遍同一次运行。',
