...
export const zh: UiDict = {
  ...
  examples: {
    ...
    entries: {
      ...
      'external-agent-process': { blurb: '无需模型 API Key，把确定性的本地子进程作为外部 Agent 运行。' },
      ...
      'eval-offline-regression': { blurb: '对两个 Target 运行无需 API Key 的 EvalSet 回归，结合规则评分器、Judge 评分器和 Gate 给出结果。' },
      'eval-online-sampling': { blurb: '对线上运行进行采样，在持久化 FileEvalStore 中异步评分，并显式关闭评估管线。' },
      'fan-out-aggregate': { blurb: '通过 AgentPool.runParallel() 的 MapReduce 式 fan-out。' },
      'multi-perspective-code-review': { blurb: '多个评审智能体并行，然后综合。' },
      'observability-v2': { blurb: '无需 API Key 的示例，覆盖批处理、TraceStore、OpenTelemetry、生命周期处理和离线 Run Viewer。' },
      'plan-replay': { blurb: '用 createPlanArtifact 固定协调器计划，再用 runFromPlan 回放，不重跑协调器。' },
      'research-aggregation': { blurb: '多源研究，由一个综合智能体汇整。' },
      'risk-gated-bash': { blurb: '将工具授权与逐次调用 Gate 组合，对 bash 命令执行放行、人工复核或拦截。' },
