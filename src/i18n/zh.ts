...
export const zh: UiDict = {
  ...
  landing: {
    ...
    faqs: [
      ...
      { q: '某个节点失败了会怎样？', a: '错误可能是暂时性问题时，失败节点会按任务策略重试。预算耗尽、输入格式错误、主动取消与不可重试的客户端错误会跳过无意义的重试。持续失败会以 FAILED 状态和错误显示在节点上，下游依赖被挂起，独立分支则可以继续。开启 recovery.mode: \'repairable\' 后，Replanner 可以在这个结果边界提出只追加的 PlanPatch，让尚未执行的部分被修订，而不是原样带着往下走。' },
