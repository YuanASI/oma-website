...
export const zh: UiDict = {
  ...
  landing: {
    ...
    hero: {
      ...
      h1: '自动分工的智能体团队，',
      h1Accent: '跑在你自己的环境里。',
      subtitle: '给它一个目标，或一张显式任务 DAG；它规划、路由并在你自己的环境里跑完这些工作。审批、预算与 trace 都在你手里，运行可从任务边界恢复。',
      audience: '面向 TypeScript 后端与 AI 平台团队',
      enterprise: '为公司定制 Agent 产品？',
      meta: ['端云模型混用', 'MIT 许可'],
      quickStart: '运行零 Key Demo',
      demoNote: '无需注册 · 无需 API Key · 不会请求模型',
      trust: { stars: 'GitHub Star', release: '最新版本', license: '许可证', downloads: '周 npm 下载' },
      runtime: {
        mode: '模式',
        modeValue: '团队',
        route: '路由',
    ...
    sectionWhy: {
      ...
      sub: '选择执行拓扑，让就绪工作立即移动，把策略放在正确边界，并保留完整证据。',
    },
    why: [
      {
        tag: '规划 → 路由',
        title: '从结果出发，也可以直接给图。',
        desc: '用 runTeam() 生成可审阅的任务 DAG，或用 runTasks() 执行你定义的图。显式 mode、治理策略或 ExecutionRouter 决定单智能体还是团队执行。',
      ...
      },
      {
        tag: '调度 → 派发',
        title: '依赖一就绪，工作就移动。',
        desc: '事件驱动调度器去掉无关的轮次屏障；按能力匹配任务、验证硬性要求、审批每次就绪派发，并保留未经合并的任务级结果。',
        proof: '事件驱动 · onTaskDispatch · taskResults',
      ...
      },
      {
        tag: '治理 → 证据',
        title: '把策略放到它真正约束的边界。',
        desc: '声明角色与审阅路径，确认高影响工具，在 turn 与 task 边界约束后续调用，再检查治理结论、运行凭证、trace 与评估闸门。',
    ...
    faqs: [
      ...
      { q: '怎样防止一次多智能体运行失控？', a: '分层控制，全部可选开启。onPlanReady 为计划设闸，onTaskDispatch 为一个就绪任务设闸（或用 onApproval 保留旧式整轮审批），onToolCall 可要求确认一次高影响操作。声明式治理会在执行后验证必要角色与顺序；runConsensus 与循环检测再补上结果和行为检查。' },
