...
export const INTEGRATIONS: Integration[] = [
  {
    ...
    seoDescription: {
      en: 'Export TraceRecord v2 spans to your own OpenTelemetry provider with @open-multi-agent/otel — core stays at three dependencies.',
      zh: '用 @open-multi-agent/otel 把 TraceRecord v2 span 导出到应用自有的 OpenTelemetry provider，core 仍保持三个依赖。',
    },
...
try {
  ...
  },
  {
    ...
    seoDescription: {
      en: 'Run external coding agents such as Claude Code inside a task DAG over ACP: config, permission boundary, and lifecycle limits.',
      zh: '通过 ACP 把 Claude Code 等外部编码智能体放进任务 DAG：配置、权限边界与生命周期限制。',
    },
...
console.log(result.success, result.totalTokenUsage)`,
  ...
  },
  {
    ...
    seoDescription: {
      en: 'Run a multi-agent team on Anthropic Claude in TypeScript: set ANTHROPIC_API_KEY, name the model, mix Claude with other providers.',
      zh: '在 TypeScript 里让多智能体团队跑在 Anthropic Claude 上：设好 ANTHROPIC_API_KEY、指定模型，并可与其它提供方混用。',
    },
...
${RUN}`,
  ...
  },
  {
    ...
    seoDescription: {
      en: 'Run a multi-agent team on AWS Bedrock in TypeScript: no API key (AWS credential chain), the Bedrock runtime SDK, and a model id.',
      zh: '在 TypeScript 里让多智能体团队跑在 AWS Bedrock 上：无需 API key（走 AWS 凭据链）、装 Bedrock runtime SDK、指定模型。',
    },
...
${RUN}`,
  ...
  },
  {
    ...
    seoDescription: {
      en: 'Point open-multi-agent at any OpenAI-compatible endpoint — Groq, OpenRouter, vLLM, LM Studio, LiteLLM — with provider and baseURL.',
      zh: '把 open-multi-agent 指向任意兼容 OpenAI 的端点——Groq、OpenRouter、vLLM、LM Studio、LiteLLM——用 provider 加 baseURL。',
    },
